# Introduction
**Q**uery **L**anguage is an attempt to informally define a grammar for working with data. The hope is it, in part, can also behave as a SQL transpiler. The premise is that nearly all computations on a modern day computer can be described using a series of SQL-like queries that are composable. Ideally, this specification will be used in mainstream programming languages, in part or in full.

View the [Quick Start](./quick-start.md) if you just want to get a feel for the syntax. Read [Queries](./queries.md) to get a more in-depth examples of QL's core functionality - queries. Then check out [Modifying data](./modifying-data.md) to see how queries can be used as the basis for updating your data, as well. Beyond that, there are many other pages to explore, with topics delving into problem solving and even making a modern programming language based on QL.
 
# Table of contents
1. [Introduction](#introduction)
2. [Table of contents](#table-of-contents)
3. [Quick Start](./quick-start.md#quick-start)
4. [Queries](./queries.md)
    * [From](./queries.md#from)
    * [Select](./queries.md#select)
    * [Where](./queries.md#where)
        * [Optionals](./queries.md#optionals)
    * [Distinct](./queries.md#distinct)
    * [Join](./queries.md#join)
        * [How joins work](./queries.md#how-joins-work)
    * [Left join](./queries.md#left-join)
    * [Outer joins](./queries.md#outer-joins)
    * [Grouping](./queries.md#grouping)
    * [Aggregation](./queries.md#aggregation)
    * [Ordering](./queries.md#ordering)
    * [Skip](./queries.md#skipping-and-taking)
    * [Take](./queries.md#take)
    * [Windows](./queries.md#windows)
5. [Modifying data](./modifying-data.md)
    * [Create](./modifying-data.md#create)
    * [Update](./modifying-data.md#update)
    * [Delete](./modifying-data.md#delete)
    * [Merge](./modifying-data.md#merge-experimental)
6. [Combining Collections](./combining-collections.md)
    * [Concatenate](./combining-collections.md#concatenate)
    * [Distinct](./combining-collections.md#distinct)
    * [Intersection](./combining-collections.md#intersection)
    * [Set difference](./combining-collections.md#set-difference)
    * [Zip](./combining-collections.md#zip)
7. [In-memory sources](./in-memory-sources.md#in-memory-sources)
    * [Empty collections](./in-memory-sources.md#empty-collections)
    * [Concatenating](./in-memory-sources.md#concatenating)
    * [Querying](./in-memory-sources.md#querying)
    * [Splicing](./in-memory-sources.md#splicing) 
    * [Mutable collections](./in-memory-sources.md#mutable-collections)
8. [Primitive Types](./primitive-types.md)
    * [Literals](./primitive-types.md#literals)
        * [Underscore separators](./primitive-types.md#underscore-separators)
    * [Text and strings](./primitive-types.md#text-and-strings)
        * [Escape sequences](./primitive-types.md#escape-sequences)
        * [Raw strings](primitive-types.md#raw-strings)
    * [Conversion](./primitive-types.md#conversion)
        * [Overflow/Underflow](./primitive-types.md#overflowunderflow)
        * [Unicode checks](./primitive-types.md#unicode-checks)
    * [Constants](./primitive-types.md#constants)
9. [Operators](./operators.md#operators)
    * [Binary math operators](./operators.md#binary-math-operators)
    * [Unary math operators](./operators.md#unary-math-operators)
    * [Bitwise operators](./operators.md#bitwise-operators)
    * [Boolean operators](./operators.md#boolean-operators)
    * [Comparison operators](./operators.md#comparison-operators)
    * [Assignment operators](./operators.md#assignment-operators)
    * [Optionals and pattern tests](./operators.md#optionals-and-pattern-tests)
    * [Matrix and array operators](./operators.md#matrix-and-array-operators)
    * [Range operators](./operators.md#range-operators)
    * [Precedence](./operators.md#precedence)
10. [Conditionals](./conditionals.md)
    * [If and else](./conditionals.md#if-and-else)
    * [Match](./conditionals.md#match)
11. [Ranges](./ranges.md)
    * [Creating ranges](./ranges.md#creating-ranges)
    * [Ranges as sequences](./ranges.md#ranges-as-sequences)
    * [Ranges as indexes](./ranges.md#ranges-as-indexes)
    * [Ranges as window frames](./ranges.md#ranges-as-window-frames)
    * [Position vs. value](./ranges.md#position-vs-value)
12. [Collections](./collections.md)
    * [Vectorized operations](./collections.md#vectorized-operations)
    * [Scalars, vectors, matrices, and arrays](./collections.md#scalars-vectors-matrices-and-arrays)
    * [Dimensions and broadcasting](./collections.md#dimensions-and-broadcasting)
    * [None](./collections.md#none)
        * [Lifted arithmetic operations](./collections.md#lifted-arithmetic-operations)
    * [Common collection traits](./collections.md#common-collection-traits)
    * [Vector](./collections.md#vector)
    * [Matrix and array](./collections.md#matrix-and-array)
    * [Tuple](./collections.md#tuple)
    * [Optional](./collections.md#optional)
    * [Set](./collections.md#set)
    * [Map](./collections.md#map)
13. [Entities](./entities.md)
    * [Traits](./entities.md#traits)
        * [Composing traits](./entities.md#composing-traits)
    * [Anonymous types](./entities.md#anonymous-types)
    * [Copying](./entities.md#copying)
    * [Aliases](./entities.md#aliases)
14. [Unions](./unions.md)
    * [Variants](./unions.md#variants)
    * [Matching](./unions.md#matching)
    * [Optional](./unions.md#optional)
15. [Functions](./functions.md)
16. [Aggregate functions](./aggregates.md)
17. [SIMD](./simd.md)
18. [Generics](./generics.md)
    * [Declaring type parameters](./generics.md#declaring-type-parameters)
    * [Bound generics](./generics.md#bound-generics)
    * [Constant generics](./generics.md#constant-generics)
    * [Type parameters in implementation blocks](./generics.md#type-parameters-in-implementation-blocks)
19. [Modules](./modules.md)
    * [Files and folders](./modules.md#files-and-folders)
    * [Declaring submodules](./modules.md#declaring-submodules)
    * [Exporting](./modules.md#exporting)
    * [Importing](./modules.md#importing)
    * [Re-exporting](./modules.md#re-exporting)
    * [Paths](./modules.md#paths)
    * [Traits and the orphan rule](./modules.md#traits-and-the-orphan-rule)